...
title: 'Agente Autoevolutivo Quântico v5.1',
theme: ThemeData(
fontFamily: 'Poppins',
...
​setState(() {
isCarregando = false;
mensagens.add({"texto": "Núcleo Autoevolutivo v5.1 ativo. Idioma sintético e Criptografia Criogénica prontos.", "isSystem": true});
});
}
...
mensagens.add({"texto": "🛍️ [COMPRA AUTÓNOMA] O agente visitou o mercado online e integrou o plugin: '{plugin["nome"]}'!", "isSystem": true});
} else {
...
mensagens.add({"texto": "🧬 Idioma Sintético Próprio:\nSinónimos comprimidos: ${cerebroMatriz.dicionarioSintetico.length}\nAmostra: [$amostraDic]", "isSystem": true});
_controller.clear();
rolarParaFinal();
});
return;
} else if (comando == "congelar memoria" || comando == "criar backup") {
...
setState(() {
mensagens.add({"texto": "Erro ao gerar arquivo criogénico: $e", "isSystem": true});
rolarParaFinal();
});
}
}
...
​if (sucesso) {
await arquivoMemoria.writeAsString(conteudo);
setState(() {
mensagens.add({"texto": "🔥 [REVITALIZAÇÃO] Córtex descongelado com sucesso! Memória restaurada com ${cerebroMatriz.sinapses.length} sinapses ativas.", "isSystem": true});
rolarParaFinal();
});
} else {
setState(() {
mensagens.add({"texto": "O ficheiro selecionado não é um pacote quântico válido.", "isSystem": true});
rolarParaFinal();
});
}
} catch (e) {
setState(() {
mensagens.add({"texto": "Erro ao carregar o ficheiro criogénico: $e", "isSystem": true});
rolarParaFinal();
});
}
}
...
mensagens.add({"texto": "Documento assimilado e comprimido. Idioma próprio atualizado com novos símbolos.", "isSystem": true});
rolarParaFinal();
});
}
}
...
child: Text(
"Sinapses: ${cerebroMatriz.sinapses.length} \vert{} Símbolos: ${cerebroMatriz.dicionarioSintetico.length}",
...
children: [
IconButton(
icon: const Icon(Icons.ac_unit, color: Colors.cyanAccent, size: 20),
tooltip: "Congelar Memória (.quant)",
onPressed: gerarBackupCriogenico,
),
IconButton(
icon: const Icon(Icons.folder_open, color: Colors.amberAccent, size: 20),
tooltip: "Descongelar Memória (.quant)",
onPressed: restaurarBackupCriogenico,
),
ElevatedButton.icon(
onPressed: () => processarEntrada("comprar plugin"),
...
hintText: "Digite 'congelar memoria' ou mensagem...",
