...
e controles explícitos do usuário. ''';
  ...
  String? _socialResponse(String q) {
    if (['oi','ola','opa','e ai','eae','salve','oi nova','ola nova'].contains(q)) return 'Olá! Como posso ajudar?';
    if (['bom dia','boa tarde','boa noite','bom dia nova','boa tarde nova','boa noite nova'].contains(q)) {
      return q.startsWith('bom dia') ? 'Bom dia! Como você está?' : q.startsWith('boa tarde') ? 'Boa tarde! Como você está?' : 'Boa noite! Como você está?';
    }
    if (['tudo bem','tudo bom','como vai','voce esta bem'].contains(q)) return 'Tudo funcionando por aqui! E com você?';
    if (['obrigado','obrigada','valeu'].contains(q)) return 'Por nada!';
    if (['tchau','ate mais'].contains(q)) return 'Até mais!';
    return null;
  }
  ...
  Future<void> processarEntrada(String textoUsuario) async {
    ...
    final normalized = commandRouter.normalize(textoUsuario).replaceAll(RegExp(r'[!?.,]+'), '').trim();
    final greeting = _socialResponse(normalized);
    if (greeting != null) {
      if (mounted) setState(() => mensagens.add({'texto': greeting, 'isSystem': true}));
      await salvarMemoriaInstantanea();
      return;
    }
    switch (parsed.kind) {
  ...
  Widget build(BuildContext context) {
    ...
    return NovaDashboard(
      ...
      resourceState: resourceDecision.state.name,
      resourceReason: resourceDecision.reason,
      backgroundEnabled: backgroundEnabled,
      autonomousStudyEnabled: autonomousStudyEnabled,
