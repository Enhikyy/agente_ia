...
class AgenteApp extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return MaterialApp(
      ...
      title: 'Agente Autoevolutivo QuÃ¢ntico v5.1',
      theme: ThemeData(
        fontFamily: 'Poppins',
...
class _MainScreenState extends State<MainScreen> {
  ...
  Future<void> arranqueBiologico() async {
    ...
    setState(() {
      isCarregando = false;
      mensagens.add({"texto": "NÃºcleo Autoevolutivo v5.1 ativo. Idioma sintÃ©tico e Criptografia CriogÃ©nica prontos.", "isSystem": true});
    });
  }
  ...
  Future<void> processarEntrada(String textoUsuario) async {
    ...
    if (comando == "comprar plugin" || comando == "ir ao shopping") {
      ...
      setState(() {
        ...
        if (resultadoCompra["sucesso"]) {
          ...
          if (!pluginsAdquiridos.contains(nomePlugin)) {
            ...
            mensagens.add({"texto": "ðŸ›ï¸ [COMPRA AUTÃ“NOMA] O agente visitou o mercado online e integrou o plugin: '$nomePlugin'!", "isSystem": true});
          } else {
      ...
      setState(() {
        ...
        mensagens.add({"texto": "ðŸ§¬ Idioma SintÃ©tico PrÃ³prio:\nSinÃ³nimos comprimidos: ${cerebroMatriz.dicionarioSintetico.length}\nAmostra: [$amostraDic]", "isSystem": true});
        _controller.clear();
        rolarParaFinal();
      });
      return;
  ...
  Future<void> gerarBackupCriogenico() async {
    try {
      ...
      setState(() {
        mensagens.add({"texto": "Erro ao gerar arquivo criogÃ©nico: $e", "isSystem": true});
        rolarParaFinal();
      });
    }
  }
    ...
    if (resultado != null && resultado.files.single.path != null) {
      try {
        ...
        if (sucesso) {
          ...
          setState(() {
            mensagens.add({"texto": "ðŸ”¥ [REVITALIZAÃ‡ÃƒO] CÃ³rtex descongelado com sucesso! MemÃ³ria restaurada com ${cerebroMatriz.sinapses.length} sinapses ativas.", "isSystem": true});
            rolarParaFinal();
          });
        } else {
          setState(() {
            mensagens.add({"texto": "O ficheiro selecionado nÃ£o Ã© um pacote quÃ¢ntico vÃ¡lido.", "isSystem": true});
            rolarParaFinal();
          });
        }
      } catch (e) {
        setState(() {
          mensagens.add({"texto": "Erro ao carregar o ficheiro criogÃ©nico: $e", "isSystem": true});
          rolarParaFinal();
        });
      }
    }
  ...
  Future<void> lerBaseDeDados() async {
    ...
    if (resultado != null && resultado.files.single.path != null) {
      ...
      setState(() {
        ...
        mensagens.add({"texto": "Documento assimilado e comprimido. Idioma prÃ³prio atualizado com novos sÃ­mbolos.", "isSystem": true});
        rolarParaFinal();
      });
    }
  }
