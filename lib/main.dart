...
e controles explícitos do usuário. ''';
  ...
  Future<void> processarEntrada(String textoUsuario) async {
    ...
    final normalized = commandRouter.normalize(textoUsuario)
        .replaceAll(RegExp(r'[!?.,]+'), '').trim();
    final greeting = _socialResponse(normalized);
    if (greeting != null) {
      if (mounted) setState(() => mensagens.add({
        'texto': greeting, 'isSystem': true,
      }));
      await salvarMemoriaInstantanea();
      return;
    }
  ...
  Widget build(BuildContext context) {
    ...
    if (RegExp(r'^(bom dia|boa tarde|boa noite)( nova)?
  ...
  Widget build(BuildContext context) {
    ...
    return NovaDashboard(
      ...
      return 'Boa ' + (normalized.startsWith('bom dia') ? 'manhã' :
        normalized.startsWith('boa tarde') ? 'tarde' : 'noite') +
        '! Como você está?';
    }
    if (RegExp(r'^(tudo bem|tudo bom|como vai|voce esta bem)( com voce)?
    if (textoUsuario.trim().isEmpty || isLendo) return;
    final parsed = commandRouter.parse(textoUsuario);
    // Every message is visible in the central chat, including quick actions.
    if (mounted) setState(() {
  ...
  Widget build(BuildContext context) {
    ...
    if (RegExp(r'^(obrigad[oa]|valeu|muito obrigad[oa])
    if (textoUsuario.trim().isEmpty || isLendo) return;
    final parsed = commandRouter.parse(textoUsuario);
    // Every message is visible in the central chat, including quick actions.
    if (mounted) setState(() {
  ...
  Widget build(BuildContext context) {
    ...
    if (RegExp(r'^(tchau|ate mais|boa noite e tchau)
    if (textoUsuario.trim().isEmpty || isLendo) return;
    final parsed = commandRouter.parse(textoUsuario);
    // Every message is visible in the central chat, including quick actions.
    if (mounted) setState(() {
